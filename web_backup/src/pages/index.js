...
const SAMPLE_SHOPS = [
  ...
  },
  {
    ...
    rating: 4.9,
    services: ['按摩', '水療', '日托服務'],
    image: 'https://images.unsplash.com/photo-1591768575198-ad40e1715d53?w=400',
  },
  {
    id: '4',
    name: 'Ruff & Fetch',
  ...
  },
  {
    ...
    rating: 4.5,
    services: ['人寵共融', '游泳池', '診所'],
    image: 'https://images.unsplash.com/photo-1599148400620-8e1ff0bf28a8?w=400',
  },
  {
    id: '6',
    name: 'WOOF MAGIC',
    ...
    <div className="relative max-w-4xl mx-auto px-4 my-4">
      <div className="relative">
        ...
        <input
          ...
          className="w-full pl-12 pr-4 py-4 bg-white rounded-xl shadow-sm border border-gray-100 focus:outline-none focus:ring-2 focus:ring-blue-500"
        />
      </div>
    </div>
  );
...
function DistrictChips({ selected, onSelect }) {
  return (
    <div className="max-w-4xl mx-auto px-4 mb-4 overflow-x-auto">
...
function AdBanner() {
  return (
    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 my-4 text-center">
      <p className="text-yellow-700 font-medium">📢 廣告位</p>
      <p className="text-yellow-600 text-sm">Google Ads</p>
    </div>
  );
}
...
function WhatsAppButton({ phone, shopName }) {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full flex items-center justify-center gap-2 bg-green-500 text-white py-4 rounded-lg font-medium hover:bg-green-600 transition"
    >
...
function MapButton({ address }) {
  ...
  return (
    <button
      onClick={handleClick}
      className="w-full flex items-center justify-center gap-2 bg-white border-2 border-blue-500 text-blue-500 py-4 rounded-lg font-medium hover:bg-blue-50 transition"
    >
      <svg className="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="p-6">
          <div className="flex items-start justify-between mb-4">
            <div>
              ...
              <p className="text-gray-500 mt-1">⭐ {shop.rating}</p>
            </div>
          </div>
          ...
          <div className="mb-6">
            <h3 className="font-semibold text-gray-800 mb-3">提供的服務</h3>
            <div className="flex flex-wrap gap-2">
              {shop.services.map((service, index) => (
                <span 
                  key={index}
                  className="px-3 py-1.5 bg-blue-50 text-blue-600 text-sm rounded-lg"
                >
                  {service}
                </span>
              ))}
...
function EmptyFavorites({ onGoHome }) {
  return (
    <div className="flex flex-col items-center justify-center py-16">
      ...
      <p className="text-gray-500 mb-4">還沒有收藏的店家</p>
      <button 
        onClick={onGoHome}
...
export default function Home() {
  ...
  const filteredShops = shops.filter(shop => {
    const matchSearch = shop.name.toLowerCase().includes(searchText.toLowerCase()) ||
                       shop.services.some(s => s.toLowerCase().includes(searchText.toLowerCase()));
    const matchDistrict = selectedDistrict === '全港' || 
                          shop.districtCn.includes(selectedDistrict) ||
                          shop.district.includes(selectedDistrict);
    return matchSearch && matchDistrict;
  });

  const toggleFavorite = (e, shop) => {
    e.stopPropagation();
    if (favorites.find(f => f.id === shop.id)) {
  ...
  const displayShops = showFavorites ? favorites : filteredShops;

  return (
    <>
      <Head>
        <title>🐾 PetGroom HK - 香港寵物美容平台</title>
        <meta name="description" content="幫助寵物主人快速找到合適的美容店家" />
      ...
      <div className="min-h-screen bg-gray-50">
        ...
        <main className="max-w-4xl mx-auto px-4 pb-8">
          {showFavorites && favorites.length === 0 ? (
            <EmptyFavorites onGoHome={() => setShowFavorites(false)} />
          ) : (
            <>
              ...
              <div className="space-y-4">
                {displayShops.map(shop => (
                  <div key={shop.id} onClick={() => setSelectedShop(shop)}>
                    ...
                    />
                    <button
                      onClick={(e) => toggleFavorite(e, shop)}
                      className="absolute right-4 mt-2 p-2"
                    >
                      <svg 
                        className={`w-6 h-6 ${
                          favorites.find(f => f.id === shop.id)
